...
name in the Quick Connect dialog, and then drag and drop files to and
...
broken, and Quick Sync provides one-click synchronization between
your local files and server.
...
you like.  The Activator tray utility reduces desktop clutter and
allows you to access any open sessions from one convenient menu.
Named sessions allow you to save preferences for individual sites.
Named firewalls can be used on a session-specific basis so that you
can work from multiple locations with different firewalls.  SecureFX
also provides support for IPv6 and the ability to run multiple
...
certificate authentication, as well as Kerberos v5 authentication
via GSSAPI.  RSA and DSA key formats are supported for host keys and
public keys.
...
The major new features for SecureFX 6.6 are:

 Enhanced control over file transfers

   - "Throttle" transfer bandwidth: Limit the bandwidth used
     during file transfers to keep from consuming all available
     bandwidth.

   - Specify number of parallel transfers: Specify the number of
     parallel file transfers and, as transfers are completed,
     queued files will continually fill available slots.

 Streamline connecting and downloading

   - Single-click download to CWD: Selecting Download on remote
     files automatically places the files in the local CWD if a
     different download location has not been specified.

   - Auto retry on failed connection: If a PASV data connection
     fails, automatically try to reconnect using original
     host/IP address.

 Enhanced compatibility

   - RFC 2228 "CCC" support: Support for RFC 2228 Clear Command
     Channel (CCC) provides enhanced compatibility with SSL
     servers.

 New UI customization options

   - Tree view display options: Additional display options for
     file sizes in tree view.

   - Upload button: Upload button can be used for menu and
     toolbar customization.

 SFXCL enhancement

   - Specify a password when using a session.

Please see HISTORY in the SecureFX program group for information on
...
Users who purchased SecureFX on or after February 1, 2009 are eligible
for a free upgrade to SecureFX 6.6.  All users can evaluate SecureFX
6.6 for 30 days free of charge.  For more information on upgrade
...
File Transfer
  ...
  - Resumed transfers complete all remaining files.
